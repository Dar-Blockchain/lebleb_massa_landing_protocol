...
class TokenData implements Serializable {
    ...
    deserialize(data: StaticArray<u8>, offset: i32): Result<i32> {
        ...
        this.price = args.nextString().expect("Failed to deserialize price");
...
export function addToken(binaryArgs: StaticArray<u8>): void {
    ...
    generateEvent('TokenAdded' + 'Token Added: Symbol: ' + symbol + ' Address: ' + address + ' Price: ' + price);
}
...
export function getPriceBySymbol(binaryArgs: StaticArray<u8>): string {
    const args = new Args(binaryArgs);
    const symbol = args.nextString().expect("Failed to parse symbol");
...
export function getPriceByAddress(binaryArgs: StaticArray<u8>): StaticArray<u8> {
    ...
    if (addressIndex.contains(address)) {
        ...
        const tokenData = tokens.getSome(symbol);
        return tokenData ? stringToBytes(tokenData.price) : stringToBytes('Price not found for token with address ' + address);
    } else {
