...
import { _balance, _setBalance, _approve, _allowance } from './lib/token-internals';
import { setOwner } from './utils/ownership';
import { u256 } from 'as-bignum/assembly';
import { _mint,_burn } from './lib/mintable/mint-internal';
...
export const NAME_KEY = stringToBytes('MToken');
export const SYMBOL_KEY = stringToBytes('MTK');
export const TOTAL_SUPPLY_KEY = stringToBytes('1000000');
export const DECIMALS_KEY = stringToBytes('18');
...
export function constructor(stringifyArgs: StaticArray<u8>): void {
  ...
  const LendingPoolAddress = args
  .nextString()
  .expect("lending address required")
  generateEvent("lendingPool "+LendingPoolAddress);
...
export function mint(binaryArgs: StaticArray<u8>): void {
  ...
  generateEvent("atoken lending"+lendingPoolAddress)
  assert(lendingPoolAddress == Context.caller().toString(), "only lending pool ")
